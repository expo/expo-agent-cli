...
The script needs a clean git checkout and the [GitHub CLI](https://cli.github.com/). From the repo root, it bumps the selected package's `package.json`, then commits, pushes, and dispatches the workflow:

```sh
bun scripts/publish.ts
bun scripts/publish.ts --package expo-agent-cli
bun scripts/publish.ts --package @expo/agent-cli
bun scripts/publish.ts --tag next
bun scripts/publish.ts 2.1.0 --package @expo/agent-cli --tag beta
