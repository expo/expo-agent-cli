...
Automatic sync uses the saved agent selection in `.expo/agent-skill-links.json` when present; otherwise it detects agents from marker directories in the project or user home. Installing a package in a freshly created app therefore links its skills for detected agents without requiring setup first. Detection is recomputed each run and is not saved. Successful project `agents:setup` with package skills enabled, or `skills:sync --agent codex`, saves an explicit selection that takes precedence over detection. Repeat `--agent` for other agents; dry runs do not save a selection. With no saved or detected agents, automatic sync does nothing.
