...
describe('@expo/agent-cli smoke', () => {
  describe('--help', () => {
    ...
    it('appears in the top-level help, next to the commands it composes', async () => {
      ...
      // Renamed twice: wave 34 tried "Check a running app", and the familiar dev
      // vocabulary was picked over it [confirmed, 2026-08-28].
  ...
  describe('against a dev server with no app', () => {
    ...
    it('opens the app with the device tool when there is a device and no app', async () => {
      ...
      expect(readXcrun()).toContainEqual([
        ...
        'openurl',
        SIMULATOR_UDID,
        // The root route needs the `?` marker for Expo Go, per llp/0005 §navigate.
  ...
  //
  // `<scheme>://<route>` navigates an app that is **already** running against a dev server;
  // `<scheme>://expo-development-client/?url=<origin>` is what loads one. A cold development build
  // handed the first sits on the dev launcher's own screen, which is what the re-test
  // photographed. This tier owns the argv, which is the whole question: which URL reached the
  ...
  describe('the URL that loads the app', () => {
    it('opens the dev-client loading link before the route link', async () => {
      ...
      try {
        ...
        const opens = readXcrun()
          .filter((argv) => argv[1] === 'openurl')
          .map((argv) => argv[3]);
    ...
    // Expo Go's own form already carries the dev server, so there is nothing to point it at first
    // and the ladder must not fire — `exp://<host>/--/<route>` *is* the loading link there.
    it('opens only the exp:// link for an Expo Go project', async () => {
      const projectRoot = await setupFixtureAsync('go-app');
      const readXcrun = await installStubXcrunAsync(projectRoot);
      const stub = await startStubDevServerAsync({ projectRoot, targets: [] });
      const release = await holdLockForAsync(projectRoot, stub);
      ...
      try {
        ...
        const opens = readXcrun()
          .filter((argv) => argv[1] === 'openurl')
          .map((argv) => argv[3]);
        expect(opens).toEqual([`exp://127.0.0.1:${stub.port}/--/?`]);
        expect(opens.join(' ')).not.toContain('expo-development-client');
      } finally {
        release();
        await stub.close();
      }
  ...
  // @ref llp/0010-agent-conventions.rfc.md §Other gates, in brief. A dev server that proved it
