...
- **The real EAS build (every PR, costs build minutes).** `agent-cli-eas-build` (`packages/@expo/agent-cli/.eas/workflows/`) scaffolds an app with `@expo/agent-cli new`, installs `expo-dev-client`, links it to the CI project, and runs a real `eas build --profile development --platform android`. This is the one path GitHub CI and the staging live tier cannot take — [[0022-live-tier]] marks native build creation `unreachable in v1` — so it lives on real EAS under `expo-ci`. The workflow runs under `EXPO_TOKEN` from the project's production environment, because the ambient workflow identity is not authorized to create a build. First green build: `ed80b512`, an APK.
