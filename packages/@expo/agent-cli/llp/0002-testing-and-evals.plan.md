...
- **Tier 1, short agent integration tests (every relevant PR, advisory).** `evals/tier1/*.eval.ts` uses Vitest 4 and `@expo/agent-eval-vitest@0.0.2`. Ollama with pinned `qwen3:8b` weights selects native CLI tool calls from public help and real results. Independent CLI-event/file/server assertions grade the outcome. Fresh projects and user profiles prevent local installations from changing fixture context. Six turns, twelve calls, a four-minute case deadline; no automatic retries or pass@k. All cases run on PRs, dispatch and the weekly diagnostic schedule. Artifacts retain model/CLI evidence even when the job fails. [implemented, 2026-09-15] The Expo Go question runs against two project states, like the impact pair: a plain real app, and the same app after it checked in its `ios` directory. The job writes a step summary table from the kit's per-case results.
- **Tier 2, long end-to-end tasks (EAS dispatch/label, advisory).** `evals/tier2` drives Claude Code headlessly on a broken, locked Expo web cart. Independent browser interactions against Metro, the agent export and a fresh export grade behavior; source/data preservation and real CLI event checks are separate requirements. Raw Claude/CLI traces, errors, diffs, screenshots and provider usage are diagnostics. The EAS workflow runs under the package base directory by `agent-cli-eval` PR label or dispatch. It preserves real failure exits and always attempts artifact upload. Missing production Claude credentials produce an explicit skip before expensive setup, not a pass. Bounds: 40 turns, $5 CLI budget, 15-minute agent / 27-minute overall deadline. [implemented, 2026-09-09] Model-free fixture/browser checks are proven locally; actual Claude completion still needs a credential-backed EAS run.
...
as a default; cadence follows measured runtime. The JSON runner remains for deterministic Tier 0; both model drivers have moved to Vitest.
