...
Keep existing template and user instructions outside the Expo managed block, except for a targeted migration of command examples: replace Expo commands wrapped by agent-cli (`install`, `start`, and `add`) and all commands in the registry's `forwardedCommands` set with the corresponding agent-cli command. Also replace `expo-doctor` (including `expo-doctor@latest`) with agent-cli `doctor`. The passthrough set is fixed rather than open-ended, so preserve unknown names. Preserve supported `npx`/`bunx` runner options and command arguments; bare commands at the start of a line or quoted example use `npx`. This removes conflicting instructions observed in real agent runs; do not migrate arbitrary user command sections. Inside the block, explicitly prefer agent-cli for equivalent Expo install/start/lint, TypeScript and expo-doctor operations (including suggestions in loaded skills), explain that installation retains SDK-compatible resolution, and preserve the `bunx` convention when `bun.lock` is present. [confirmed, Kudo, 2026-09-08; expanded command migration, 2026-09-09]
