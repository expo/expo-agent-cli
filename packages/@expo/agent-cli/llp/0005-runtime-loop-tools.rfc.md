...
`src/status/__tests__/installed-test.ts`: that disabled lookups read no device, that every platform this host can reach is asked, that no phone is named unless the caller named one, and the shape of the section.
