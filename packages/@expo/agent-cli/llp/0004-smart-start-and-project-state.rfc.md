...
The check asks the device instead. A debug build on SDK 55 or later carries the fingerprint it was
...
<secs>` bounds how long the phone gets to answer once the app was launched, 15 s by default. A named
device widens the section's deadline by the launch's own budget and that answer time.
...
The app id comes from the static app config (`ios.bundleIdentifier`, `android.package`), then from
the prebuilt project (`android/app/build.gradle` `applicationId`, `ios/*.xcodeproj`
`PRODUCT_BUNDLE_IDENTIFIER` of the Debug configuration, with the app target picked out of any
extension targets). Expo Go is never the target: it carries no project fingerprint. A project that
names no id for a platform reports `app-id-unknown` for it.
...
version rule in all three shapes (equal hashes across versions, differing hashes across versions, a
...
`src/runtime/__tests__/appId-test.ts`: the pbxproj read over an app with extension targets and over a
Debug/Release pair.
