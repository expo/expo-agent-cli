...
**A `--wait-ready` that ran out is rule 2 as well, and its report used to depend on which of two processes was quicker — F153.** `waitForBundlerReadyAsync` reports one fact — this bundler did not answer inside the budget — and two very different runs produce it. In one the plan is compiling and nothing has gone wrong, which is what `stillBuildingError` is for. In the other the child is **gone**, and there every sentence of that wording is false: nothing is "still running", no build is "still going", and the reason it stopped is in the child's log with a scenario name and an "Ask the user" line on it. Which one a caller got was decided by whether the parent's last `/status` probe was slower than the child's death, and under load it was not [observed — `dev-detach-test.ts`, exit 20 in place of exit 7 with 48 busy processes, 2026-09-06]. So the ordering is no longer read: the wait ends the moment the child does, and the child's own verdict is consulted before any not-ready wording is chosen. Two orderings had to go rather than one, and the second was found by the fix's own CI run [observed — windows-2022, 2026-09-06]. "The child is gone" was read off the parent's `exit` event alone, which is an observation that can lag the process it is about, so it is now read off the pid the lock names as well — `isProcessAlive` answers on every platform and touches nothing. And "the child's verdict" was taken from the first thing `parseDetachedChildVerdict` could answer, which is the `Error:` line: `logCmdError` prints the failure and the handoff block in two writes, so a reader between them saw a stop with no scenario and reported "the process exited" for one that had a name and an "Ask the user" line a beat behind it. The wait now ends on a scenario, and its deadline is what ends it otherwise. What the wait may still be failed _on_ is the two facts that are conclusive without a readiness claim — a handoff block in the log, and a child that is gone — for the same reason the grace window is bounded to them: a bundler that has not answered yet is the ordinary state of a first compile.
