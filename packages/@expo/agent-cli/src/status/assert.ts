...
export function buildAssertStatus(
  ...
  if (actual == null) {
    return {
      ...
      reason: `No class could be established, so "${asserted}" was not verified — ${undecidedCause(freshness)}. A gate that passed on an answer nothing measured would not be a gate; run "${PROGRAM_PREFIX} status --explain" to see what is missing, or "${PROGRAM_PREFIX} dev --ios|--android" once to record a build to compare against.`,
    };
  }
