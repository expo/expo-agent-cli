...
describe('the old name', () => {
  ...
  it(`is gone from llp/, with no exception left`, () => {
    // The reservation record was the last allowed survivor, and it was dropped on purpose:
    // `@expo/agent-cli` is the final name and the only one [confirmed, 2026-09-02].
    expect(scan(['llp'])).toEqual([]);
  });
