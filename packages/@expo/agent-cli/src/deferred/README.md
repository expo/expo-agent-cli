...
- **vitest** — `exclude` in `vitest.config.ts`. The suites moved here with their code and
...
- **the suggested-command lint** — `SKIPPED_DIRECTORIES` in `src/lint/sweep.ts`. Every file here
  ...
  the point of the exclusion is that the lint keeps catching it _everywhere else_.
