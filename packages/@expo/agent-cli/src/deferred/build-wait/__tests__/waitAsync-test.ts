...
const spawn = spawnSubprocessAsync as MockedFunction<typeof spawnSubprocessAsync>;
...
async function waitAsync(waitOptions: BuildWaitOptions) {
  ...
  await vi.advanceTimersByTimeAsync(waitOptions.timeoutMs + waitOptions.maxIntervalMs);
  return settled;
}
