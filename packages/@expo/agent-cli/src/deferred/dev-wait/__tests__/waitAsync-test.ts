...
vi.mock('../../runtime/waitReady', () => ({
  waitForBundlerReadyAsync: vi.fn(),
...
function mockBundle(overrides: Partial<BundleCheckResult> = {}) {
  vi.mocked(checkEntryBundleAsync).mockResolvedValue({
    outcome: 'ok',
    platform: 'ios',
...
describe(devWaitAsync, () => {
  ...
  describe('--require-app', () => {
    it(`should wait for an app on what is left of the budget`, async () => {
      ...
      const [, appOptions] = vi.mocked(waitForAppConnectionAsync).mock.calls[0]!;
    ...
    it(`should not wait for an app when the bundle was never built`, async () => {
      ...
      expect(vi.mocked(waitForAppConnectionAsync)).not.toHaveBeenCalled();
    });
  ...
  describe('the entry-bundle check', () => {
    ...
    it(`should build the entry bundle once the dev server is ready`, async () => {
      ...
      const [url, checkOptions] = vi.mocked(checkEntryBundleAsync).mock.calls[0]!;
    ...
    it(`should not build anything with --no-bundle-check`, async () => {
      ...
      expect(vi.mocked(checkEntryBundleAsync)).not.toHaveBeenCalled();
  ...
  it(`should emit the answer on the event stream, whatever the output mode`, async () => {
    ...
    expect(vi.mocked(event)).toHaveBeenCalledWith('dev_wait', {
      devServerUrl,
      source: 'lock',
      ready: true,
      projectRootMatched: true,
...
describe(`${devWaitAsync.name} and the platform it builds for`, () => {
  ...
  it(`builds for the platform the connected app is on, not for the default`, async () => {
    ...
    expect(vi.mocked(checkEntryBundleAsync).mock.calls[0]![1]).toMatchObject({
      platform: 'android',
    });
  ...
  it(`builds for both when apps on both platforms are connected`, async () => {
    ...
    mockReady();
    vi.mocked(checkEntryBundleAsync)
  ...
  it(`still builds for exactly the platform a --platform flag named`, async () => {
    ...
    expect(vi.mocked(checkEntryBundleAsync).mock.calls[0]![1]).toMatchObject({ platform: 'web' });
