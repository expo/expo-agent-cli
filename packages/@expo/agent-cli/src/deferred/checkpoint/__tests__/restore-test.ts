...
vi.mock('../git', async () => ({
  ...
  diffTreesAsync: vi.fn(),
  restoreTreeAsync: vi.fn(),
}));
...
beforeEach(() => {
  ...
  vi.mocked(writeSnapshotTreeAsync).mockResolvedValue({ tree: 'current-tree', files: 12 });
  vi.mocked(objectExistsAsync).mockResolvedValue(true);
  vi.mocked(restoreTreeAsync).mockResolvedValue(undefined);
  vi.mocked(diffTreesAsync).mockResolvedValue([
...
describe(undoAsync, () => {
  ...
  it(`should report a checkpoint whose git object is gone`, async () => {
    seedCheckpoints([record()]);
    vi.mocked(objectExistsAsync).mockResolvedValue(false);
