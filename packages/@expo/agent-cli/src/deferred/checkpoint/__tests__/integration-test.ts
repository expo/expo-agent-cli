...
function warned(): string {
  return vi.mocked(Log.warn).mock.calls.flat().join('\n');
}

beforeEach(() => {
  delete process.env.AGENT_CLI_NO_CHECKPOINT;
  vi.mocked(createCheckpointAsync).mockResolvedValue(created());
});
...
describe(checkpointBeforeAsync, () => {
  ...
  it(`should say nothing when the project is not in a git repository`, async () => {
    vi.mocked(createCheckpointAsync).mockResolvedValue({
      record: null,
      files: 0,
