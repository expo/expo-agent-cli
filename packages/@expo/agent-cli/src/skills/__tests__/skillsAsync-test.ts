...
vi.mock('../../log');
vi.mock('../../utils/agent', () => ({
  getAgentTelemetryContext: vi.fn(() => null),
}));
vi.mock('../discovery', () => ({ discoverSkillsAsync: vi.fn() }));
vi.mock('../agents', () => ({
  getAllAgents: vi.fn(),
  detectInstalledAgentsAsync: vi.fn(),
  getPersistedAgentIdsAsync: vi.fn(),
  resolveAgentsAsync: vi.fn(),
  persistAgentSelectionAsync: vi.fn(),
}));
vi.mock('../linking', () => ({
  syncSkillLinksAsync: vi.fn(),
  cleanSkillLinksAsync: vi.fn(),
  updateGitIgnoreAsync: vi.fn(),
}));
...
describe('listSkillsAsync with json output', () => {
  ...
  it('should print machine readable skill metadata', async () => {
    ...
    const output = vi.mocked(Log.log).mock.calls.at(-1)?.[0];
  ...
  it('should print an empty json array when nothing is discovered', async () => {
    vi.mocked(discoverSkillsAsync).mockResolvedValueOnce([]);
    vi.mocked(getPersistedAgentIdsAsync).mockResolvedValueOnce(null);
    vi.mocked(detectInstalledAgentsAsync).mockResolvedValueOnce([]);
...
describe('showSkillsAsync', () => {
  ...
  it('should filter to a single skill by name', async () => {
    ...
    vi.mocked(discoverSkillsAsync).mockResolvedValueOnce([testSkill, secondSkill]);
...
describe('skills --json reports', () => {
  ...
  function parsed(): any {
    const calls = vi.mocked(Log.log).mock.calls;
  ...
  it('should report the swept directories and the removed links for a clean', async () => {
    vi.mocked(cleanSkillLinksAsync).mockResolvedValueOnce({ pruned: ['.claude/skills/gone'] });
...
describe('cleanSkillsAsync', () => {
  ...
  it('should clean managed links from all known agent directories', async () => {
    ...
    vi.mocked(cleanSkillLinksAsync).mockResolvedValueOnce({ pruned: [] });
...
describe('printSkillsForAgentAsync', () => {
  ...
  it('should do nothing when no agent is detected', async () => {
    vi.mocked(getAgentTelemetryContext).mockReturnValueOnce(null);
  ...
  it('should print skill contents of the installed packages', async () => {
    ...
    const output = vi.mocked(Log.log).mock.calls.flat().join('\n');
