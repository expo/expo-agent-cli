...
vi.mock('../changedFiles', () => ({ listChangedFilesAsync: vi.fn() }));
...
describe(refineWithChangedFilesAsync, () => {
  ...
  beforeEach(() => {
    vi.mocked(listChangedFilesAsync).mockReset();
  });
  ...
  it(`should answer null rather than throwing when git blows up`, async () => {
    vi.mocked(listChangedFilesAsync).mockRejectedValue(new Error('git exploded'));
  ...
  it(`should give up on its deadline rather than holding the report`, async () => {
    vi.useFakeTimers();
    try {
      vi.mocked(listChangedFilesAsync).mockReturnValue(new Promise(() => {}));
