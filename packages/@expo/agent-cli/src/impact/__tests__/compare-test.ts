...
beforeEach(() => {
  ...
  vi.mocked(readLastBuildRecord).mockReset().mockReturnValue({});
  vi.mocked(generateFingerprintAsync).mockReset();
  vi.mocked(diffFingerprintsAsync).mockReset();
});

describe(compareWithLastBuildAsync, () => {
  it(`should fingerprint the working tree for the platform and the preset asked for`, async () => {
    vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'head', sources: [source] });
  ...
  it(`should say a v1 record can answer whether, and not what`, async () => {
    vi.mocked(readLastBuildRecord).mockReturnValue({ ios: { hash: 'base', sources: null } });
    vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'head', sources: [source] });
  ...
  it(`should diff a v2 record against the head`, async () => {
    vi.mocked(readLastBuildRecord).mockReturnValue({ ios: { hash: 'base', sources: [source] } });
    vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'head', sources: [source] });
    vi.mocked(diffFingerprintsAsync).mockResolvedValue({ items: realDiff as any });
  ...
  it(`should report an unchanged fingerprint with an empty diff`, async () => {
    vi.mocked(readLastBuildRecord).mockReturnValue({ ios: { hash: 'same', sources: [source] } });
    vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'same', sources: [source] });
    vi.mocked(diffFingerprintsAsync).mockResolvedValue({ items: [] });
  ...
  it(`should keep the hash answer when the diff itself failed`, async () => {
    vi.mocked(readLastBuildRecord).mockReturnValue({ ios: { hash: 'base', sources: [source] } });
    vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'head', sources: [source] });
    vi.mocked(diffFingerprintsAsync).mockResolvedValue({ items: null, error: 'diff failed' });
...
describe(compareWithEasBuildAsync, () => {
  it(`should spawn eas with the compare argv, in the project`, async () => {
    ...
    expect(spawnSubprocessAsync).toHaveBeenCalledWith(
      easCli.command,
      spawned(buildEasCompareArgs('build-1')),
      {
        cwd: projectRoot,
        output: 'capture',
      }
