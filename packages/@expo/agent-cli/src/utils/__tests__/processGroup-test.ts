...
function mockSpawnSync(status: number | null): Mock {
  const fn = vi.mocked(spawnSync);
  fn.mockReturnValue({ status } as ReturnType<typeof spawnSync>);
  return fn;
}
...
describe(killProcessTree, () => {
  ...
  it(`falls back to the direct kill when taskkill cannot run`, () => {
    ...
    vi.mocked(spawnSync).mockImplementation(() => {
      throw new Error('spawnSync missing');
    });
...
describe('the spawn that makes a group possible', () => {
  it(`puts every subprocess in a process group of its own`, async () => {
    ...
    expect((vi.mocked(spawn).mock.calls[0]![2] as any).detached).toBe(USE_PROCESS_GROUP);
  });
});
