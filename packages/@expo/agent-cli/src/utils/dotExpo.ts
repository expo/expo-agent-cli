...
- "agent-cli-eas-builds.json": remembers what "${PROGRAM_NAME} status --explain" learned from EAS about this project's current fingerprint, so the next "${PROGRAM_NAME} status" can report it without calling EAS again. A finished build it found is believed until the project fingerprint changes; an answer of "no such build" is believed for five minutes, and the report says how old it is. Pass "--no-fingerprint-cache" to ask EAS again.
- "agent-cli-app-config.json": remembers what "expo config --type public" evaluated a dynamic "app.config.js" or "app.config.ts" to, so "${PROGRAM_NAME} status" does not spawn it again for an unchanged project. Revalidated the same way as the fingerprint record below, and never believed for longer than ten minutes. A static "app.json" is never remembered here: it is read directly.
