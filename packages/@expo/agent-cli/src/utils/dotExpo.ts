...
- "dev/logs/build-ios.log" and "dev/logs/build-android.log": everything the last native build "${PROGRAM_NAME} dev" ran for that platform printed, in the order it arrived, so "${PROGRAM_NAME} inspect:build-log --local --ios" can say what failed in it afterwards. Written only when no terminal was watching the run (a detached run included), and truncated on each build.
