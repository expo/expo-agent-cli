// Read one entry out of a zip archive, such as an asset inside an APK, without unpacking the whole
// file. The three structures it needs — the end-of-central-directory record, the central directory
// and the entry's local header — can each come from a ranged read, so a caller can fetch a few
// hundred KB of a device file instead of pulling the whole APK. ZIP64 archives are not supported.
...
/**
 * Read a single entry from a zip archive held in memory. Supports stored and deflated entries.
 * Returns null when the entry does not exist. Throws on a malformed archive.
 */
...
/**
 * Locate the central directory from the end-of-central-directory record. `tail` must hold the final
 * bytes of the file; at most the last `EOCD_MAX_LENGTH` bytes are scanned. Returned offsets are
 * relative to the start of the whole file.
 */
export function parseEndOfCentralDirectory(tail: Buffer): ZipCentralDirectory {
  ...
  for (let offset = tail.length - 22; offset >= scanStart; offset--) {
    ...
    // The signature bytes can occur inside the archive comment. Only the real record's
    // comment-length field matches the number of bytes between the record's end and the file's end.
    ...
    if (entryCount === 0xffff) {
      // 0xffff is a ZIP64 sentinel and also a legal entry count. The ZIP64 EOCD locator, 20 bytes
      // before this record, tells them apart. A buffer too short to hold one is refused.
...
/**
 * Find an entry in a central directory. `centralDirectory` must start at the directory's first
 * byte, which a ranged read of `ZipCentralDirectory.offset` and `size` provides.
 */
...
/**
 * Decompress an entry from a buffer that starts at its local file header. The buffer must span
 * the header and the compressed data; `LOCAL_HEADER_MAX_LENGTH + compressedSize` bytes always
 * suffice.
 */
export function readLocalZipEntry(
  ...
  // The sizes in the local header may be zero when a data descriptor follows the data, so the
  // central directory's size is the one that is trusted.
