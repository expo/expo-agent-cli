...
vi.mock('../../utils/inheritedRun', () => ({ runInheritedAsync: vi.fn() }));
...
describe(`register passthrough`, () => {
  ...
  it(`should spawn the expo CLI with register, and say so on stderr`, async () => {
    vi.spyOn(process, 'cwd').mockReturnValue(path.resolve('/elsewhere'));
    vi.mocked(runInheritedAsync).mockResolvedValue(0);

    await agentCliPassthrough('register')([]);

    expect(runInheritedAsync).toHaveBeenCalledWith('npx', ['expo', 'register'], expect.anything());
    expect(vi.mocked(Log.error).mock.calls.join('\n')).toContain('npx expo');
  });
...
describe(agentCliPassthrough, () => {
  it(`should route the four auth commands away from the plain expo forward`, async () => {
    ...
    vi.spyOn(process, 'cwd').mockReturnValue(path.resolve('/elsewhere'));
    vi.spyOn(subprocess, 'spawnSubprocessAsync').mockResolvedValue({
      exitCode: 0,
      stdout: '',
      stderr: '',
    });
