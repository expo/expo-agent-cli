...
export interface IosDevice {
  ...
  /** Paired, with a live tunnel. An unplugged, unpaired or disconnected device cannot be launched on. */
  reachable: boolean;
}
...
/** Read the device list out of the JSON `devicectl` writes. */
export function parseDevicectlList(json: unknown): IosDevice[] {
  ...
  for (const device of devices) {
    ...
    found.push({
      ...
      reachable: connection?.pairingState === 'paired' && connection?.tunnelState === 'connected',
    });
  }
  return found;
}
