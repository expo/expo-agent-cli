 ...
 *
 * `await vi.importActual` because this suite mocks `fs` with `memfs`, and these are the one thing in
...
function recorded(name: string): string {
  return realFs.readFileSync(path.join(__dirname, '..', '..', '__fixtures__', 'eas', name), 'utf8');
}
