...
vi.mock('../impactReport', () => ({ reportInstallImpactAsync: vi.fn() }));
vi.mock('../../skills/skillsAsync', () => ({
  autoSyncSkillsAsync: vi.fn(),
  listSkillPackagesAsync: vi.fn(),
  printSkillsForAgentAsync: vi.fn(),
}));
...
beforeEach(() => {
  ...
  vi.mocked(reportInstallImpactAsync).mockResolvedValue([report()]);
  vi.mocked(listSkillPackagesAsync).mockResolvedValue([]);
});
...
describe(installAsync, () => {
  ...
  describe('follow-ups', () => {
    ...
    it(`should warn that the running app cannot load a new native module`, async () => {
      vi.mocked(reportInstallImpactAsync).mockResolvedValue([
        report({
    ...
    it(`should point at the skill of a package that ships one`, async () => {
      vi.mocked(listSkillPackagesAsync).mockResolvedValue(['@expo/ui']);
