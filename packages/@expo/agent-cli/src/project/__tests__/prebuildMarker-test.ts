...
import { formatChangedSources } from '../sourceDiff';
...
describe(getPrebuildStaleness, () => {
  ...
  it(`is stale and names the project sources that moved, project first`, () => {
    ...
    expect(staleness.changes).toEqual([
      ...
      { source: 'the app config', change: 'changed', scope: 'project' },
      { source: 'node_modules/expo-camera/plugin', change: 'changed', scope: 'dependency' },
    ]);
    expect(formatChangedSources(staleness.changes)).toBe('plugins/withFoo.js, the app config');
  });
  ...
  it(`is stale on a dependency-only change, with nothing to name`, () => {
    ...
    expect(staleness.status).toBe('stale');
    expect(formatChangedSources(staleness.changes)).toBe('');
  });
  ...
  it(`truncates a long list of project sources`, () => {
    ...
    expect(formatChangedSources(changes)).toBe('a.js, b.js, c.js, and 2 more');
  });
...
describe(getNativeDirectoryStaleness, () => {
  ...
  it(`reads the platform's own marker file`, () => {
    ...
    expect(getNativeDirectoryStaleness(projectRoot, 'android', current)).toMatchObject({
      status: 'stale',
      changes: [{ source: 'the app config', change: 'changed' }],
    });
  });
...
describe(readPrebuildMarker, () => {
  it(`reads a marker this CLI wrote`, () => {
  ...
  // One rejection per field of the schema, so a file that is not a marker is never believed.
  it.each([
...
describe('recordPrebuildMarkersAsync', () => {
  ...
  it('removes an old marker when its replacement cannot be computed', async () => {
    ...
    await recordPrebuildMarkersAsync(projectRoot, ['-p', 'ios'], {
      generateFingerprint: fakeFingerprint('old'),
      clearMemo: vi.fn(),
    });
