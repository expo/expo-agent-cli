...
describe(diffSources, () => {
  it('uses package versions and contents override keys as source identities', () => {
    ...
    ).map((item) => item.change)).toEqual(['removed', 'added']);
    expect(diffSources(
...
describe(describeSource, () => {
  it.each(['node_modules/pkg/a', 'node_modules\\pkg\\a', '../pkg/a', '..\\pkg\\a'])(
    'recognizes dependency path %s', (filePath) => {
  ...
  it('excludes React Native contents and names common project inputs', () => {
    expect(describeSource({ type: 'contents', id: 'package:react-native' }).scope).toBe('dependency');
    expect(describeSource({ type: 'contents', id: 'expoAutolinkingConfig:ios' }).source)
      .toBe('Expo autolinking configuration');
    expect(describeSource({ type: 'contents', id: 'packageJson:scripts' }).source)
      .toBe('package.json scripts');
  });
});
