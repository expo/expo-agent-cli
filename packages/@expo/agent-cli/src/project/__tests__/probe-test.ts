...
vi.mock('../expoGo', () => ({ checkExpoGoCompatibilityAsync: vi.fn() }));
vi.mock('../fingerprint', () => ({ generateFingerprintAsync: vi.fn() }));
...
beforeEach(() => {
  vi.mocked(checkExpoGoCompatibilityAsync).mockResolvedValue(compatible);
  vi.mocked(generateFingerprintAsync).mockResolvedValue({ hash: 'abc123', sources: null });
});
...
describe(probeProjectStateAsync, () => {
  ...
  it(`should pass the Expo Go compatibility result through`, async () => {
    ...
    vi.mocked(checkExpoGoCompatibilityAsync).mockResolvedValue(incompatible);
    writeProject({ expo: '54.0.0' });
