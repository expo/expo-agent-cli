...
//
// `expo prebuild` records nothing, so this CLI does: the `prebuild` passthrough and the `dev` plan
// write the marker after a prebuild that exited 0, and the installed-app check reads it. A prebuild
// run outside this CLI leaves no marker, which reads as `unknown` — never as fresh.
 ...
 * (autolinking, patch-package patches, `eas.json`, `package.json` scripts) change the build
 * without making the generated directories stale.
 */
...
/** Read one platform's marker, or null when there is none to believe. Never throws. */
 ...
 *
 * Best effort throughout: a hash that cannot be computed, or a file that cannot be written, leaves
 * no marker, and no marker reads as `unknown` — never as fresh.
 *
...
export async function recordPrebuildMarkersAsync(
  ...
  const recorded = await Promise.all(
    platformsToRecord(projectRoot, args).map(async (platform) => {
