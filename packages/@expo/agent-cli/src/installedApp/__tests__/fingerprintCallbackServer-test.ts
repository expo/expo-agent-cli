...
import { isPrivateIPv4, startFingerprintCallbackServerAsync } from '../fingerprintCallbackServer';
...
describe(startFingerprintCallbackServerAsync, () => {
  ...
  it(`reads a missing fingerprint version as null`, async () => {
    ...
    try {
      await post(
        loopbackUrl(server.callbackUrl),
        JSON.stringify({ nonce: 'abc', fingerprint: 'h' })
      );
      server.armTimeout();
...
// The responder refuses a callback to anything but a private or link-local IPv4, so the address
// advertised has to be one of those or the probe can only time out.
describe(isPrivateIPv4, () => {
  it.each([
    ...
    '100.64.0.1',
    '169.254.10.10',
  ])(`accepts %s`, (address) => expect(isPrivateIPv4(address)).toBe(true));
  it.each(['8.8.8.8', '172.32.0.1', '100.128.0.1', '203.0.113.7', '::1', 'fe80::1', 'not-an-ip'])(
    `refuses %s`,
