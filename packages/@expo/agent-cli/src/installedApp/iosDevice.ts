...
import {
  ...
  PHONE_LAUNCH_TIMEOUT_MS,
  type IosDevice,
} from '../device/devicectl';
import * as Log from '../log';
import { extendSubprocessDeadline } from '../utils/subprocessDeadline';
import { startFingerprintCallbackServerAsync } from './fingerprintCallbackServer';
import { buildFingerprintCheckUrl, DEFAULT_RESPONSE_TIMEOUT_MS } from './fingerprintCheckProtocol';
import {
  matchesDeviceFilter,
  pickBestResult,
...
export async function readInstalledFingerprintIosDeviceAsync({
  ...
  for (const candidate of usable) {
    ...
    Log.progress(`Checking ${device.name}. This launches the app on the device.`);
...
async function probeDeviceAsync(
  ...
  // The enclosing `status` deadline was sized for file reads. A launch and a phone's answer are
  // paid for here, once a phone is really about to be launched on.
  extendSubprocessDeadline(PHONE_LAUNCH_TIMEOUT_MS + (timeoutMs ?? DEFAULT_RESPONSE_TIMEOUT_MS));
  ...
  try {
    ...
    try {
      ...
      // routes by scheme, so without one the launch — which may still have delivered the trigger —
      // is all there is, and the answer is waited for below.
      if (scheme) {
        try {
