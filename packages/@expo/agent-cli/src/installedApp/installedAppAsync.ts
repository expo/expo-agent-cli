...
export async function checkInstalledAppAsync(
  ...
  const deps = { readInstalled, generateFingerprint, readAppId, readFingerprintVersion };
    ...
    case 'ok': {
      ...
      if (
        ...
        return verdict('fingerprint-version-mismatch', {
          ...
          // No command: `commands` is what an agent runs, and the sentence above says not to.
          commands: [],
        });
      }
