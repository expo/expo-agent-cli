...
/**
 * Whether an IPv4 address is one the responder accepts a callback to: RFC 1918, the carrier-grade
 * NAT block, or link-local. The Swift side refuses anything else (expo/expo#49494), so advertising
 * a VPN or public address would only time out.
 */
...
export function resolvePrivateLanHost(): string | null {
 ...
 *
 * @throws {CommandError} `NO_LAN_ADDRESS` when this machine has no private IPv4 address a phone can reach.
 */
export async function startFingerprintCallbackServerAsync({
  nonce,
  timeoutMs = DEFAULT_RESPONSE_TIMEOUT_MS,
  lanHost = resolvePrivateLanHost,
  ...
  if (!host) {
    throw new CommandError(
      'NO_LAN_ADDRESS',
      [
        `This computer has no private LAN address, so a physical device cannot reach it to report its fingerprint.`,
        `Why: the device posts the fingerprint back over Wi-Fi, and its responder only accepts a private (RFC 1918, 100.64/10 or 169.254/16) IPv4 address — a VPN or public address is refused.`,
