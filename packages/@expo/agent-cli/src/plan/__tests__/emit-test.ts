...
describe(emitStartPlan, () => {
  ...
  it(`should print the plan as JSON and nothing else`, () => {
    ...
    expect(JSON.parse(vi.mocked(Log.log).mock.calls[0]![0]!)).toEqual({
      ...plan,
      followups: [],
    });
  });
  ...
  it(`should print a stable set of top-level keys with --json`, () => {
    ...
    const printed = JSON.parse(vi.mocked(Log.log).mock.calls[0]![0]!);
  ...
  it(`should embed the follow-ups in the JSON plan`, () => {
    ...
    expect(JSON.parse(vi.mocked(Log.log).mock.calls[0]![0]!).followups).toEqual(followups);
  });
  ...
  it(`should print an empty follow-up list when none is passed`, () => {
    ...
    expect(JSON.parse(vi.mocked(Log.log).mock.calls[0]![0]!).followups).toEqual([]);
  });
