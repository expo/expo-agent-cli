...
describe(navigateAsync, () => {
  ...
  it(`should print one JSON object and nothing else with --json`, async () => {
    ...
    expect(JSON.parse(printed())).toEqual({
      ...
      // (llp/0005-runtime-loop-tools.rfc.md §Android, F50).
      attached: null,
      attachWaitedMs: 0,
  ...
  // @ref llp/0005-runtime-loop-tools.rfc.md §navigate
  describe('--print-url', () => {
    function mockExpoGoProject() {
      vol.fromJSON({
        [`${projectRoot}/package.json`]: JSON.stringify({ name: 'demo', dependencies: {} }),
...
// @ref ../adbReverse, llp/0005-runtime-loop-tools.rfc.md §Android — friction run 6, F50. `am start` exits 0 for an intent
