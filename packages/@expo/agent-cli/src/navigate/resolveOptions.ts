...
export interface NavigateOptions {
  ...
  printUrl: boolean;
  /**
   * Which device backends this run may use, decided by `--eas`.
   *
...
export function resolveNavigateOptions(argv: string[]): NavigateOptions {
  ...
  // that asks for **no** device, and `--eas` names one. A run that took both would have to
  // either open the link (ignoring `--print-url`) or not (ignoring `--eas`).
  if (args['--eas'] && args['--print-url']) {
    throw new CommandError(
      'BAD_ARGS',
      [
        `--eas and --print-url ask for opposite things, so this run has no rule for what to do.`,
        `Why: --eas says to open the link on an EAS Simulator session, and --print-url says to open nothing and print the URL instead.`,
        `How: pass one. Use --eas to drive the session, or --print-url to get the URL and hand it to whatever opens it.`,
      ].join('\n')
    );
  }
