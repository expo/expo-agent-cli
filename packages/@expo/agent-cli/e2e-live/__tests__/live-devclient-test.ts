...
describeDevClient('live-devclient: the loop on a real Android development build', () => {
  ...
  it.each([
    ...
    const result = await runLiveAsync(
      run,
      projectRoot,
      ['runtime:tap', testID, '--android', '--json'],
      {
  ...
  // @ref llp/0022-live-tier.plan.md §live-android — the row this
  ...
  it('loads a development build that is not running, then navigates it', async () => {
    ...
    // a coin toss ([[0022-live-tier]] §What a live assertion may be).
