...
Everything needs the bundle built first: **`bun run build`**.
...
- **An AVD that boots without Expo Go on it fails the suite rather than skipping it.** By then the boot
  has been spent, and vitest cannot turn a running suite into a skipped one. The message names the fix:
...
_present_, not which SDK it is, because the scaffold's SDK does not exist until `beforeAll` scaffolds.
...
```bash
bun run build                               # the tier tests this artifact, so it must exist

bun run test:live:project                   # ~45 s, free — needs only the network
bun run test:live:local                     # ~1 min, free
