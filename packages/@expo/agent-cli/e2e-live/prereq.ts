...
//
// Every gate in this file is **synchronous**, and that is the whole design constraint: vitest decides
 ...
 * avoid. vitest prints skipped suites without their reason, so the reason is printed here.
 */
export function describeLive(name: string, gate: Gate): typeof describe | typeof describe.skip {
  if (gate.ok) {
    return describe;
  }
 ...
 * `bun run build`, and a stale or absent one is the one prerequisite failure that would make the whole
...
export function builtBinGate(): Gate {
  ...
  return fs.existsSync(bundle)
    ? ok
    : missing(`the ncc bundle is not built (${bundle} does not exist) — run "bun run build" first`);
}
