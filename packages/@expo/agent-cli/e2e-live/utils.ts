 ...
 *
 * A class rather than module state so two suites in one vitest process cannot see each other's
...
export class LiveRun {
   ...
   *
   * A `LiveRun` is constructed in the `describe` body, which vitest evaluates **even for a suite it is
   ...
   * a throw from a `describe` body is reported by vitest as a module-load failure with no test names
...
export async function runLiveAsync(
  ...
  // a bare `spawn`:
  //
  //  - `npm_config_minimum_release_age`, inherited from the repo's bunfig.toml, blocks
  //    recently published packages — and what the registry serves is this tier's subject.
  //  - **`NODE_ENV` and `VITEST_*`.** vitest sets `NODE_ENV=test`, and a live child that inherits
 ...
 * of a bundler's opinion, and a vitest assertion message that long is unreadable in a terminal.
 */
