...
import { addCheckedInNativeProject } from '../harness/prebuilt-fixture';
import { projectFiles } from '../harness/results';
import { snapshot } from '../harness/workspace';

// The twin of expo-go: same prompt and dependencies, but this project checked in its ios directory.
// A model that always answers yes passes the other case and fails here.
agentEval(
  import.meta.url,
  {
  ...
  (check) => {
    check('inspects this project’s Expo Go compatibility', (_workspace, { fixture }) => {
      ...
      ).toContainEqual(expect.objectContaining({ expoGoCompatible: false, sdkVersion: '57.0.19' }));
    });
