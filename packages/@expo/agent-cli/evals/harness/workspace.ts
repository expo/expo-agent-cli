...
export function copyWorkspace(fixture: string, linkDependencies = false, destination?: string) {
  ...
  const root = destination ?? fs.mkdtempSync(path.join(os.tmpdir(), 'agent-cli-eval-'));
  try {
