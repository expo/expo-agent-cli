...
describe(nativeMessages, () => {
  it('should replay the CLI result as a tool response, not a new user instruction', () => {
    expect(
      nativeMessages([
      ...
      },
      {
        role: 'tool',
        tool_name: 'run_cli',
        content: '{"exitCode":0,"stdout":"linked","stderr":""}',
      },
    ]);
  });

  it('should replay a native batch with one tool result per invocation', () => {
