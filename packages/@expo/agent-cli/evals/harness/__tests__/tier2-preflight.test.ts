...
describe('Running the Tier 2 preflight and report scripts', () => {
  it('should report missing credentials as skipped before any installed dependencies are needed', () => {
    const root = mkdtempSync(join(tmpdir(), 'tier2-preflight-test-'));
    const target = join(root, 'evals/tier2');
    const artifacts = join(root, 'evals/.artifacts/tier2');
    ...
    try {
      ...
      const output = JSON.parse(readFileSync(join(artifacts, 'outcome.json'), 'utf8'));
      ...
      expect(
        JSON.parse(readFileSync(join(artifacts, 'workflow-summary.json'), 'utf8')).result
      ).toBe('skipped');
      // A later runner failure or missing status must still fail even with a skipped outcome.
      const exitFile = join(artifacts, 'runner-exit-code');
